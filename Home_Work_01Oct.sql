...
-- (1) Find students who scored more than 80 marks.
SELECT * FROM Students WHERE marks > 80;

-- (2) List students from Pune.
SELECT *  FROM Students WHERE city = "Pune";

-- (3) Show students who scored between 60 and 90.
SELECT * FROM Students WHERE marks BETWEEN 60 AND 90;

-- (4) Get all students who study Science or Math.
SELECT * FROM Students WHERE course IN ("Science" AND "Math");

-- (5) Find students whose name ends with “n”.
SELECT *
FROM Students
WHERE name LIKE '%n';
...
-- Questions
-- (1) Find all products with price greater than 10,000.
SELECT * FROM Products WHERE price > 10000;

-- (2) List all furniture items.
 SELECT *
FROM products
...
-- (4) Find items whose name contains “a”.
SELECT * FROM Products WHERE name LIKE "%a%";

-- (5) Show products in category Electronics or Stationery.
...
-- Questions
-- (1) Find orders placed after June 2023.
SELECT * FROM Orders WHERE order_date > '2023-06-30';

-- (2) List all customers who bought more than 2 items.
SELECT * FROM Orders WHERE quantity > 2;

-- (3) Show orders where product is Laptop or Mobile.
SELECT * FROM Orders WHERE product IN ("Laptop", "Mobile");

-- (4) Find orders by customer whose name starts with “A”.
SELECT * FROM Orders WHERE customer_name LIKE "A%";

-- (5) Show all orders not containing “Book”.
...
-- Questions
-- (1) Find all books priced above 400.
SELECT * FROM Library WHERE price > 400;

-- (2) List books of genre Fiction.
SELECT * FROM Library WHERE genre = "Fiction";

-- (3) Show books written by authors whose name starts with “R”.
SELECT * FROM Library WHERE author LIKE "R%";

-- (4) Find books not in genre Programming.
SELECT * FROM Library WHERE genre <> "Programming";

-- (5) Show books with price between 300 and 600.
